...
public class WidgetTemplate extends SkinnableContainer implements IWidgetTemplate
{
    ...
    public function WidgetTemplate()
    {
		...
		addEventListener(Event.RESIZE,wtResize);
    }
	
	private function wtResize(event : Event):void{
		var wt : WidgetTemplate = event.target as WidgetTemplate;
		var bounds:Rectangle = new Rectangle(0, 0, stage.stageWidth - 20, stage.stageHeight - 20);
		if (wt.height <= bounds.top + 45)
			wt.height = bounds.top;
		else if (wt.height >= bounds.bottom - 50)
			wt.height = bounds.bottom - 50;
		if(wt.width >= bounds.right - 40)
			wt.width = bounds.right - 35;
	 ...
	 * 主动触发widget的Move事件
	 */ 
	public function minOpen():void {
		var event:MoveEvent = new MoveEvent(MoveEvent.MOVE);
		_baseWidget.dispatchEvent(event);
	}
}
