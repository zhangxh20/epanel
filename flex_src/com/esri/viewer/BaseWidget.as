...
public class BaseWidget extends Module implements IBaseWidget
{
    ...
    public function BaseWidget()
    {
		...
		addEventListener(MoveEvent.MOVE,widget_moveHandler);
    }
	
	private function widget_moveHandler(event:MoveEvent):void {
		var widget:BaseWidget = event.currentTarget as BaseWidget;
		var bounds:Rectangle = new Rectangle(0, 0, stage.stageWidth, stage.stageHeight);
		var windowBounds:Rectangle = widget.getBounds(stage);
		windowBounds.width = widget.width;
		var x:Number;
		var y:Number;
		if (windowBounds.left <= bounds.left)
			x = bounds.left;
		else if (windowBounds.right >= bounds.right)
			x = bounds.right - widget.width;
		else
			x = widget.x;
		if (windowBounds.top <= bounds.top+45)
			y = bounds.top+40;
		else if (windowBounds.bottom >= bounds.bottom)
			y = bounds.bottom - widget.height;
		else
			y = widget.y;
		widget.move(x, y);
	}
